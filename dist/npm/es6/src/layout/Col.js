  ...
  Col.prototype.render = function render() {
    ...
    ['span', 'offset', 'pull', 'push'].forEach(function (prop) {
      if (_this2.props[prop] >= 0) {
        classList.push(prop !== 'span' ? 'r-el-col-' + prop + '-' + _this2.props[prop] : 'r-el-col-' + _this2.props[prop]);
      }
    });
    ...
    ['xs', 'sm', 'md', 'lg'].forEach(function (size) {
      if (_typeof(_this2.props[size]) === 'object') {
        ...
        Object.keys(props).forEach(function (prop) {
          classList.push(prop !== 'span' ? 'r-el-col-' + size + '-' + prop + '-' + props[prop] : 'r-el-col-' + size + '-' + props[prop]);
        });
      } else if (_this2.props[size] >= 0) {
        classList.push('r-el-col-' + size + '-' + Number(_this2.props[size]));
      }
    });
